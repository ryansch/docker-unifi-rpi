...
GOSS_PATH="${HOME}/.local/bin/goss-linux-arm" GOSS_WAIT_OPTS='-r 200s -s 1s > /dev/null' dgoss run ryansch/unifi-rpi:${docker_version}-arm32v7
# Testing arm64v8 is blocked on cross compiling goss for arm64
# GOSS_PATH="${HOME}/.local/bin/goss-linux-arm" GOSS_WAIT_OPTS='-r 100s -s 1s > /dev/null' dgoss run ryansch/unifi-rpi:${docker_version}-arm64v8
